...
if(isset($_GET['modif'])){
	// Ajouter recette
							...
							while ($rec = $categories->fetch()){
								...
								//On affiche les lignes une à une:
								while ($rec = $recettes->fetch()){
										echo '<option value="' . $rec['id_recette'] .','.$rec['nom_recette'].'">' . $rec['nom_recette'].'</option>';
								};   
...
<!-- Permet de stocker les variables nom_menu et nb_recettes -->
...
// A changer
// Pourquoi nom_menu.value ?? nom_menu.value vide au départ
// Trouver autre manière de récup nom_menu
...
function ajouter_recette(){
	...
	// création d'un nouveau input hiden avec value=id_recette
